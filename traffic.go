...
func createExplorerMapKey(entry TrafficAnalysis) string {
	...
	key = key + strconv.Itoa(entry.ExpSrv.Port) + entry.ExpSrv.Process + strconv.Itoa(entry.ExpSrv.Proto) + entry.ExpSrv.User + entry.ExpSrv.WindowsService + strconv.FormatFloat(entry.NumConnections, 'f', precision, 64) + entry.PolicyDecision + entry.Src.FQDN + entry.Src.IP
